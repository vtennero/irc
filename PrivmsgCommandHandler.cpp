...
void PrivmsgCommandHandler::handle(Client& sender, const Message& message) {
	...
	if (toLowerCase(target) == "nickserv") {
		runAuth(sender, content);
		return;
	}
	...
	if (target[0] == '#') {
		...
		sender.send(":" + server.getServerName() + " NOTICE " + sender.getNickname() + " :Message sent to channel " + target + "\r\n");
		return;
	}
	...
	if (targetClient) {
		...
		// Send acknowledgment to sender
		sender.send(":" + server.getServerName() + " NOTICE " + sender.getNickname() + " :Message sent to " + target + "\r\n");
	} else {
